...
async function writeMcpProvenance(filePath: string): Promise<void> {
  await fs.writeFile(
    `${filePath}.ruler-generated`,
...
describe('revert-engine', () => {
  ...
  describe('cleanUpAuxiliaryFiles', () => {
    it('should remove additional files and empty directories', async () => {
      ...
      await writeMcpProvenance(geminiSettings);
      await writeMcpProvenance(mcpFile);
      await writeMcpProvenance(junieMcpFile);
    ...
    it('does not treat gitignore-only MCP entries as generated provenance', async () => {
      const mcpFile = path.join(tmpDir, '.mcp.json');
      const originalContent = '{"mcpServers": {"user": {"command": "node"}}}';
