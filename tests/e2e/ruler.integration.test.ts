...
describe("Comprehensive Integration Test: skiller init → setup → apply → verify", () => {
	...
	it("executes complete CLI workflow and inspects all generated files", async () => {
		...
		// Step 1: Run skiller init
		console.log("\n1. Running skiller init...");
		...
		const initAgentsFile = path.join(skillerDir, "AGENTS.md");
		const initTomlFile = path.join(skillerDir, "skiller.toml");
		...
		console.log("✓ skiller init completed successfully");

		// Step 2: Create custom skiller.toml with 2 MCP servers (stdio and remote)
		console.log("\n2. Creating custom skiller.toml with MCP servers...");
		const customTomlContent = `# Skiller Configuration File
...
`;
		...
		console.log(
			"✓ Custom skiller.toml created with stdio and remote MCP servers",
		);
...
This is a CLI tool called "Skiller" that helps manage AI coding assistant configurations across different tools and platforms.
...
`;
		...
		const extraMdFile = path.join(skillerDir, "extra-rules.md");
...
This content should appear in concatenated agent files after running skiller apply.
File: extra-rules.md
`;
		...
		// Step 5: Run skiller apply
		console.log("\n5. Running skiller apply...");
		...
		for (const file of markdownRuleFiles) {
			if (fileContents[file]) {
				if (file === "CLAUDE.md") {
					...
					expect(fileContents[file]).toContain("@.claude/AGENTS.md");
					expect(fileContents[file]).toContain("@.claude/extra-rules.md");
		...
		// Verify .gitignore contains skiller entries
		if (fileContents[".gitignore"]) {
			...
			console.log(
				"✓ .gitignore contains comprehensive Skiller-generated entries",
			);
		}
