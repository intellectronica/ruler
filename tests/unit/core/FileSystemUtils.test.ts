...
describe('FileSystemUtils', () => {
  ...
  describe('backupFile', () => {
    it('does not overwrite an existing backup', async () => {
      ...
      await expect(
        fs.readFile(`${filePath}.bak.ruler-generated`, 'utf8'),
      ).resolves.toContain('Generated by Ruler');
    });

    it('refuses to use an existing unverified backup', async () => {
      const filePath = path.join(tmpDir, 'unverified-backup-target.txt');

      await fs.writeFile(filePath, 'original');
      await fs.writeFile(`${filePath}.bak`, 'stale backup');

      await expect(backupFile(filePath)).rejects.toThrow(
        'Refusing to use existing unverified backup file',
      );
    ...
    it('does not back up a file with sidecar Ruler provenance', async () => {
      const filePath = path.join(tmpDir, 'sidecar-generated-target.json');
      await fs.writeFile(filePath, '{"mcpServers":{}}\n');
      await writeGeneratedProvenance(filePath);
