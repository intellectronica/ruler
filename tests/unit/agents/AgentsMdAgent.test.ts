...
/**
 * Tests for AgentsMdAgent pseudo-agent which writes concatenated rules to `.claude/AGENTS.md`.
 */
describe("AgentsMdAgent", () => {
	...
	it("writes rules when file does not exist", async () => {
		...
		expect(written).toBe(`<!-- Generated by Skiller -->\n${rules}`);
	...
	it("creates backup and writes new content when content changes", async () => {
		...
		await agent.applySkillerConfig(initial, tmpDir, null);
		const modified = "Modified rules content";
		await agent.applySkillerConfig(modified, tmpDir, null);
		const written = await fs.readFile(targetFile, "utf8");
		expect(written).toBe(`<!-- Generated by Skiller -->\n${modified}`);
		const backup = await fs.readFile(`${targetFile}.bak`, "utf8");
		expect(backup).toBe(`<!-- Generated by Skiller -->\n${initial}`);
	});
