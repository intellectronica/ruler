...
describe('OpenCode Agent Integration', () => {
  ...
  it('writes marked instructions idempotently', async () => {
    const rulerDir = path.join(tmpDir, '.ruler');
    await fs.mkdir(rulerDir, { recursive: true });
    ...
    const firstContent = await fs.readFile(instructionsPath, 'utf8');
    ...
    expect(firstContent).toMatch(/^<!-- Generated by Ruler -->\n/);
