...
async function writeBackupProvenance(backupPath: string): Promise<void> {
  await fs.writeFile(
    `${backupPath}.ruler-generated`,
    `${GENERATED_MARKER}\nBackup generated by Ruler\n`,
  );
}
...
describe('Revert Core Functions', () => {
  ...
  describe('revertAllAgentConfigs', () => {
    ...
    it('should keep consumed backup when keep-backups is true', async () => {
      ...
      await writeBackupProvenance(backupPath);
    ...
    it('does not restore an unverified pre-existing backup', async () => {
      const filePath = path.join(tmpDir, 'CLAUDE.md');
      const backupPath = `${filePath}.bak`;

      await fs.writeFile(backupPath, 'Stale backup content');
    ...
    it('respects child default_agents when reverting nested outputs', async () => {
      const childRoot = path.join(tmpDir, 'packages', 'child');
      await fs.mkdir(path.join(childRoot, '.ruler'), { recursive: true });
      await fs.writeFile(
        path.join(tmpDir, '.ruler', 'ruler.toml'),
      ...
      await expect(fs.access(path.join(tmpDir, 'CLAUDE.md'))).rejects.toThrow();
      await expect(
        fs.access(path.join(childRoot, 'AGENTS.md')),
      ).rejects.toThrow();
    });

    it('does not clean auxiliary files for agents excluded by config selection', async () => {
      ...
      await fs.writeFile(claudePath, `${GENERATED_MARKER}\nClaude content`);
      await fs.writeFile(cursorMcpPath, '{"mcpServers":{}}\n');
      await fs.writeFile(
        `${cursorMcpPath}.ruler-generated`,
        `${GENERATED_MARKER}\nMCP config generated by Ruler\n`,
      );
      ...
      await expect(fs.access(cursorMcpPath)).resolves.toBeUndefined();
    });
    ...
    it('processes shared MCP files once during a multi-agent revert', async () => {
      ...
      await writeBackupProvenance(`${mcpPath}.bak`);
      await fs.writeFile(
        path.join(tmpDir, '.gitignore'),
        [
          '# START Ruler Generated Files',
