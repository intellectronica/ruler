...
const RULER_SOURCE_MARKER_PREFIX = '<!-- Source: .ruler/';
...
async function hasRulerGeneratedProvenance(
  ...
  try {
    ...
    if (content.trimStart().startsWith(RULER_SOURCE_MARKER_PREFIX)) {
      return true;
    }
  } catch {
    return false;
...
async function removeGeneratedFile(
  ...
  if (
    ...
    logVerbose(
      `Preserving file without backup or Ruler provenance: ${filePath}`,
      verbose,
    );
    return false;
  }
