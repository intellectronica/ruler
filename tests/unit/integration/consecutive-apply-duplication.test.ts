...
describe('Consecutive applies duplication prevention', () => {
  ...
  beforeEach(async () => {
    tmpDir = await fs.mkdtemp(
      path.join(os.tmpdir(), 'ruler-consecutive-test-'),
    );
    projectRoot = tmpDir;
  ...
  it('should not duplicate content in windsurf output on consecutive applies', async () => {
    ...
    const initialRulerContent =
      'Initial ruler instructions from .ruler directory.';
    await fs.writeFile(
    ...
    expect(concatenatedRules).not.toMatch(
      /Initial ruler instructions.*Initial ruler instructions/s,
    );
  ...
  it('should handle multiple consecutive applies without exponential growth', async () => {
    ...
    for (let i = 0; i < 3; i++) {
      ...
      await agentsMdAgent.applyRulerConfig(
        concatenatedRules,
        projectRoot,
        null,
      );
      await windsurfAgent.applyRulerConfig(
        concatenatedRules,
        projectRoot,
        null,
      );
