...
const RULER_PROVENANCE_SUFFIX = '.ruler-generated';
...
export function getGeneratedProvenancePath(filePath: string): string {
  return `${filePath}${RULER_PROVENANCE_SUFFIX}`;
}

export async function writeGeneratedProvenance(
  filePath: string,
  containmentRoot?: string,
  label = 'File',
): Promise<void> {
  await writeGeneratedFile(
    getGeneratedProvenancePath(filePath),
    `${RULER_GENERATED_MARKER}\n${label} generated by Ruler\n`,
    containmentRoot,
  );
}
...
export interface ReadMarkdownFilesOptions {
  ...
  /**
   * When true, prepend the repository root `AGENTS.md` outside `.ruler`.
   * Defaults to true only for local `.ruler` directories, and false for the
   * global XDG `ruler` config directory.
   */
  includeRootAgents?: boolean;
...
export async function readMarkdownFiles(
  ...
  const includeRootAgents =
    options.includeRootAgents ?? path.basename(rulerDir) === '.ruler';
  ...
  if (includeRootAgents) {
    // Prepend repository root AGENTS.md (outside .ruler) if it exists and is not identical path.
    try {
...
export async function backupFile(
  ...
  const backupProvenancePath = getGeneratedProvenancePath(backupPath);
  if (containmentRoot) {
  ...
  try {
    ...
    await writeGeneratedProvenance(backupPath, containmentRoot, 'Backup');
  } catch (error) {
    if ((error as NodeJS.ErrnoException).code === 'ENOENT') {
      return;
    }
