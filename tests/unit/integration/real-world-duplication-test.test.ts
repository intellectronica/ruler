...
describe("Real-world duplication bug reproduction", () => {
	...
	beforeEach(async () => {
		tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "skiller-real-test-"));
		projectRoot = tmpDir;
	...
	it("should simulate the real CLI workflow with consecutive applies", async () => {
		// Step 1: Create initial skiller content like a real project would have
		...
		// Also create a basic skiller.toml config
		await fs.writeFile(
			path.join(skillerDir, "skiller.toml"),
			"[agents.windsurf]\nenabled = true",
		);

		// Step 2: First apply - simulate `skiller apply`
		console.log("\n=== FIRST APPLY ===");
		...
		expect(secondAgentsMdContent).toContain("<!-- Generated by Skiller -->");

		// The content should only contain the original content once
		const skillerContentCount = (
			secondAgentsMdContent.match(/helpful coding assistant/g) || []
		).length;
		expect(skillerContentCount).toBe(1);
	});
});
