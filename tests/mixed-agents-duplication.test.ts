...
describe("AGENTS.md + Agent Duplication Fix", () => {
	...
	beforeEach(async () => {
		tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "skiller-mixed-test-"));
	});
...
`;
		...
		console.log(
			"AGENTS.md after codex apply:",
			agentsMdContent1.substring(0, 200),
		);
		console.log(
			"AGENTS.md starts with marker?",
			agentsMdContent1.startsWith("<!-- Generated by Skiller -->"),
		);
		...
		console.log(
			"Windsurf (AGENTS.md) content after apply:",
		...
		console.log(
			"Windsurf (AGENTS.md) content after 2nd apply:",
		...
		console.log(
			`Windsurf instruction occurrences: 1st=${instructionMatches1}, 2nd=${instructionMatches2}`,
		);
		console.log(
			`Content lengths: 1st=${windsurfContent1.length}, 2nd=${windsurfContent2.length}`,
		);
	...
	it("reproduces exact user scenario with skiller folder", async () => {
		console.log(`Test directory: ${tempDir}`);
...
`;
		...
		// Run skiller apply equivalent multiple times for different agents
		const agents = ["codex", "windsurf", "copilot", "gemini", "cursor"];

		for (const agent of agents) {
			console.log(`\n=== Testing ${agent} ===`);

			// Clean up previous agent files but keep .claude
			...
			try {
				...
				const instructionMatches = (
					agentsContent.match(/# Default Agent Instructions/g) || []
				).length;
				console.log(
					`${agent} AGENTS.md instruction count: ${instructionMatches}`,
				);
				console.log(
					`${agent} AGENTS.md starts with marker: ${agentsContent.startsWith("<!-- Generated by Skiller -->")}`,
				);
			...
			if (agentSpecificPath) {
				try {
					...
					const instructionMatches = (
						agentContent.match(/# Default Agent Instructions/g) || []
					).length;
					console.log(
						`${agent} specific file instruction count: ${instructionMatches}`,
					);
					if (instructionMatches > 1) {
