...
describe('generateSkillsFromRules', () => {
  ...
  it('generates skills from .mdc files with frontmatter', async () => {
    ...
    await fs.mkdir(path.join(rulerDir, 'rules'), { recursive: true });
...
`;
    ...
    await fs.writeFile(
      path.join(rulerDir, 'rules', 'ai.mdc'),
      mdcContent,
    );

    // Generate skills
    await generateSkillsFromRules(projectRoot, rulerDir, false, false);

    // Check that skill was generated
    const skillFile = path.join(rulerDir, 'skills', 'ai', 'SKILL.md');
    const skillContent = await fs.readFile(skillFile, 'utf8');

    // Verify frontmatter
    expect(skillContent).toContain('name: ai');
    expect(skillContent).toContain('description: Use when working on AI features');
    expect(skillContent).toContain('Applies to files matching: **/*.ts, **/*.tsx');

    // Verify @filename reference
    expect(skillContent).toContain('@.ruler/rules/ai.mdc');
  });
...
`;
    ...
    await fs.writeFile(
      path.join(rulerDir, 'rules', 'plain.mdc'),
      mdcContent,
    );
    ...
    const skillFile = path.join(rulerDir, 'skills', 'plain', 'SKILL.md');
...
`;
    ...
    await fs.writeFile(
      path.join(rulerDir, 'rules', 'security.mdc'),
      mdcContent,
    );
    ...
    const skillFile = path.join(rulerDir, 'skills', 'security', 'SKILL.md');
    await expect(fs.access(skillFile)).rejects.toThrow();
  });
...
`;
    ...
    await fs.writeFile(
      path.join(rulerDir, 'rules', 'context.mdc'),
      mdcContent,
    );
    ...
    // Check that skill WAS generated
    const skillFile = path.join(rulerDir, 'skills', 'context', 'SKILL.md');
    const skillContent = await fs.readFile(skillFile, 'utf8');
    ...
    expect(skillContent).toContain('@.ruler/rules/context.mdc');
  });
...
`;
    ...
    await fs.writeFile(
      path.join(rulerDir, 'rules', 'test.mdc'),
      mdcContent,
    );

    // Generate skills in dry-run mode
    await generateSkillsFromRules(projectRoot, rulerDir, false, true);

    // Check that no skill was actually created
    const skillFile = path.join(rulerDir, 'skills', 'test', 'SKILL.md');
    await expect(fs.access(skillFile)).rejects.toThrow();
  });
...
`;
    ...
    await fs.writeFile(
      path.join(rulerDir, 'rules', 'dynamic.mdc'),
      mdcContent1,
    );
    ...
    const skillFile = path.join(rulerDir, 'skills', 'dynamic', 'SKILL.md');
    await expect(fs.readFile(skillFile, 'utf8')).resolves.toContain('@.ruler/rules/dynamic.mdc');
...
`;
    ...
    await fs.writeFile(
      path.join(claudeDir, 'rules', 'claude-rule.mdc'),
      mdcContent,
    );
    ...
    const skillFile = path.join(claudeDir, 'skills', 'claude-rule', 'SKILL.md');
    const skillContent = await fs.readFile(skillFile, 'utf8');
