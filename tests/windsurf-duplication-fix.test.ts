...
describe("Windsurf Duplication Fix", () => {
	...
	beforeEach(async () => {
		tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "skiller-windsurf-test-"));
	});
	...
	it("should not duplicate content on consecutive windsurf applies", async () => {
		...
		// Create .claude directory and instructions file exactly as user described
...
`;
		...
		console.log("Created .claude/instructions.md");
		...
		try {
			...
			console.log(
				"Windsurf content preview:",
				windsurfContent1.substring(0, 200),
			);
		} catch (error) {
		...
		try {
			...
			console.log(
				"AGENTS.md content length after 2nd apply:",
				agentsMdContent2.length,
			);
		} catch (error) {
		...
		try {
			...
			console.log(
				"Windsurf content length after 2nd apply:",
				windsurfContent2.length,
			);
			console.log(
				"Windsurf content preview after 2nd apply:",
				windsurfContent2.substring(0, 200),
			);
		} catch (error) {
		...
		const instructionMatches1 = (
			windsurfContent1.match(/# Default Agent Instructions/g) || []
		).length;
		...
		console.log(
			`Instruction occurrences: 1st=${instructionMatches1}, 2nd=${instructionMatches2}`,
		);
