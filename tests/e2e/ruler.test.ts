...
describe("End-to-End Skiller CLI", () => {
	...
	beforeEach(async () => {
		...
		await fs.rm(path.join(projectRoot, ".cursor"), {
			recursive: true,
			force: true,
		});
		await fs.rm(path.join(projectRoot, ".windsurf"), {
			recursive: true,
			force: true,
		});
		await fs.rm(path.join(projectRoot, ".clinerules"), { force: true });
		await fs.rm(path.join(projectRoot, ".aider.conf.yml"), { force: true });
		await fs.rm(path.join(projectRoot, ".junie"), {
			recursive: true,
			force: true,
		});
		await fs.rm(path.join(projectRoot, ".idx"), {
			recursive: true,
			force: true,
		});
		...
		// Reset the TOML config to default state (remove custom config but recreate empty one)
		await fs.rm(path.join(projectRoot, ".claude", "skiller.toml"), {
			force: true,
		});
		await fs.writeFile(path.join(projectRoot, ".claude", "skiller.toml"), "");
		// Clean up Open Hands agent files
		await fs.rm(path.join(projectRoot, ".openhands"), {
			recursive: true,
			force: true,
		});
		await fs.rm(path.join(projectRoot, "config.toml"), { force: true });
		await fs.rm(path.join(projectRoot, ".kilocode"), {
			recursive: true,
			force: true,
		});
	});
	...
	it("generates configuration files for all agents", () => {
		...
		return Promise.all([
			expect(fs.readFile(claudePath, "utf8")).resolves.toContain(
				"@.claude/b.md",
			),
			...
			expect(fs.readFile(openHandsInstructionsPath, "utf8")).resolves.toContain(
				"Rule A",
			),
	...
	it("respects default_agents in config file", async () => {
		...
		await fs.writeFile(
			path.join(testProject.projectRoot, ".claude", "skiller.toml"),
			toml,
		);
		...
		).resolves.toContain("@.claude/b.md");
	});
	...
	it("CLI --agents overrides default_agents", async () => {
		...
		runSkillerWithInheritedStdio("apply --agents codex", testProject.projectRoot);
		await expect(
			fs.readFile(path.join(testProject.projectRoot, "AGENTS.md"), "utf8"),
		).resolves.toContain("Rule A");
		await expect(
	...
	it("CLI --agents firebase creates .idx/airules.md", async () => {
		...
		await expect(fs.readFile(firebasePath, "utf8")).resolves.toContain(
			"Rule A",
		);
		await expect(fs.readFile(firebasePath, "utf8")).resolves.toContain(
			"Rule B",
...
`;
		...
		await expect(
			fs.readFile(
				path.join(testProject.projectRoot, "custom_cursor.md"),
				"utf8",
			),
		).resolves.toContain("Rule A");
	});
...
`;
	...
	describe("gitignore integration", () => {
		it("creates .gitignore with generated file paths by default", async () => {
			...
			expect(gitignoreContent).toContain("# START Skiller Generated Files");
			expect(gitignoreContent).toContain("# END Skiller Generated Files");
...
output_path = "custom-claude.md"`;
	...
	describe("gitignore generation for all configs", () => {
		...
		beforeAll(async () => {
			...
			await fs.mkdir(path.join(gitignoreTestProject.projectRoot, ".vscode"), {
				recursive: true,
			});
			await fs.mkdir(path.join(gitignoreTestProject.projectRoot, ".gemini"), {
				recursive: true,
			});
			await fs.mkdir(path.join(gitignoreTestProject.projectRoot, ".cursor"), {
				recursive: true,
			});

			// Run the command that is being tested
			runSkillerWithInheritedStdio("apply", gitignoreTestProject.projectRoot);
