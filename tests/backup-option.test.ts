...
describe('backup option', () => {
  ...
  it('does not create .bak files in nested mode when [backup] enabled=false in TOML', async () => {
    const customProject = await setupTestProject({
      '.ruler/AGENTS.md': '# Test Rules\n\nSome test rules here.',
      '.ruler/ruler.toml':
        'default_agents = ["claude"]\nnested = true\n\n[backup]\nenabled = false\n',
      'CLAUDE.md': '# Existing file\nThis should not be backed up.',
    });

    try {
      const { projectRoot } = customProject;
      const backupFile = path.join(projectRoot, 'CLAUDE.md.bak');

      runRuler('apply --nested', projectRoot);
