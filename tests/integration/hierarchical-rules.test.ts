...
describe("Nested Rules Integration", () => {
	...
	it("processes each .claude directory independently in nested mode", async () => {
		...
		await fs.writeFile(
			path.join(testProject.projectRoot, ".claude", "AGENTS.md"),
...
`,
		...
		await fs.writeFile(
			path.join(submoduleDir, ".claude", "AGENTS.md"),
			"# Submodule Rules\n\nThese are submodule-level rules that should only appear in submodule-level files.",
		);

		await fs.writeFile(
			path.join(submoduleDir, ".claude", "skiller.toml"),
